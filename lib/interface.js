...
Interface.prototype.sendMessageWithData = function (endpointArn, message, callback) {
    var converterMsg;
    if (this.platform === 'android') {
        converterMsg = convertToGcmFormat(message.alert);
    } else if (this.platform === 'ios') {
        converterMsg = convertToApnsFormatWithData(message.alert, message.data, this.sandbox);
    } else {
    ...
    this.sns.publish({
        Message: JSON.stringify(converterMsg),
...
function convertToApnsFormatWithData(message, data, sandbox) {
